...
public class PaymentController implements Initializable {
    ...
    private void save() {
        ...
        if(!amount.matches(AMOUNT_REGEX)) {
            ...
            try {
                ...
                boolean result = paymentBO.savePayment(paymentDTO);
                if (result) {
    ...
    private void update() {
        ...
        if(!amount.matches(AMOUNT_REGEX)) {
            ...
            try {
                ...
                boolean result = paymentBO.updatePayment(paymentDTO);
                if (result) {
                    Alert alert = new Alert(Alert.AlertType.INFORMATION);
                    alert.setTitle("Payment Management");
    ...
    private void handleSearchPayment(KeyEvent event) {
        if (event.getCode() == KeyCode.ENTER) {
            try {
                ...
                PaymentDTO paymentDTO = paymentBO.searchPayment(id);
    ...
    private void loadMemberID(){
        try {
            ObservableList<MemberDTO> idList = paymentBO.loadMemberID();
