...
public class TimeSlotController implements Initializable {
    ...
    private void save() {
        ...
        if(!timeIn.matches(TIME_REGEX)) {
            ...
            try {
                ...
                boolean result = timeSlotDAO.save(slotDTO);
                if (result) {
    ...
    private void update() {
        ...
        if(!timeIn.matches(TIME_REGEX)) {
            ...
            try {
                ...
                boolean result = timeSlotDAO.update(slotDTO);
                if (result) {
                    Alert alert = new Alert(Alert.AlertType.INFORMATION);
    ...
    private void delete() {
        ...
        try {
            boolean result = timeSlotDAO.delete(id);
            if (result) {
    ...
    private void handleSearchTimeSlot(KeyEvent event) throws SQLException {
        ...
        if (event.getCode() == KeyCode.ENTER) {
            try {
                ...
                TimeSlotDTO slotDTO = timeSlotDAO.search(id);
    ...
    public void loadTimeSlotTable() {
        try {
            ...
            List<TimeSlotDTO> slotList = timeSlotDAO.getAllTimeSlot();
