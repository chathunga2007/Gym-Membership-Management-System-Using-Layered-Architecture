...
public class TimeSlotController implements Initializable {
    ...
    private void save() {
        ...
        if(!timeIn.matches(TIME_REGEX)) {
            ...
            try {
                ...
                boolean result = timeSlotBO.saveTimeSlot(slotDTO);
                if (result) {
    ...
    private void update() {
        ...
        if(!timeIn.matches(TIME_REGEX)) {
            ...
            try {
                ...
                boolean result = timeSlotBO.updateTimeSlot(slotDTO);
                if (result) {
                    Alert alert = new Alert(Alert.AlertType.INFORMATION);
    ...
    private void delete() {
        ...
        try {
            boolean result = timeSlotBO.deleteTimeSlot(id);
            if (result) {
    ...
    private void handleSearchTimeSlot(KeyEvent event) throws SQLException {
        ...
        if (event.getCode() == KeyCode.ENTER) {
            try {
                ...
                TimeSlotDTO slotDTO = timeSlotBO.searchTimeSlot(id);
    ...
    public void loadTimeSlotTable() {
        try {
            ...
            List<TimeSlotDTO> slotList = timeSlotBO.getAllTimeSlots();
