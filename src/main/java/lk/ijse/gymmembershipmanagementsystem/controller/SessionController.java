...
public class SessionController implements Initializable {
    ...
    private final SessionBO sessionBO = (SessionBO) BOFactory.getInstance().getBO(BOFactory.BOType.SESSION);
    private final TrainerBO trainerBO = (TrainerBO) BOFactory.getInstance().getBO(BOFactory.BOType.TRAINER);
    private final TimeSlotBO timeSlotBO = (TimeSlotBO) BOFactory.getInstance().getBO(BOFactory.BOType.TIME_SLOT);
    private final EquipmentBO equipmentBO = (EquipmentBO) BOFactory.getInstance().getBO(BOFactory.BOType.EQUIPMENT);
    ...
    private void save() {
        ...
        if(!duration.matches(DURATION_REGEX)) {
            ...
            try {
                ...
                boolean result = sessionBO.saveSession(sessionDTO, equipmentIDs);
    ...
    private void update() throws SQLException {
        ...
        try {
            ...
            boolean result = sessionBO.updateSession(sessionDTO);
            if (result) {
                Alert alert = new Alert(Alert.AlertType.INFORMATION);
    ...
    private void delete() throws SQLException {
        ...
        try {
            boolean result = sessionBO.deleteSession(id);
            if (result) {
    ...
    private void handleSearchSession(KeyEvent event) throws SQLException {
        if (event.getCode() == KeyCode.ENTER) {
            try {
                ...
                SessionDTO sessionDTO = sessionBO.searchSession(id);
    ...
    private void loadTrainerID(){
        try {
            ObservableList<TrainerDTO> idList = trainerBO.loadTrainerIDs();
    ...
    private void loadSlotID(){
        try {
            ObservableList<TimeSlotDTO> idList = timeSlotBO.loadTimeSlotIDs();
    ...
    public void loadSessionTable() {
        try {
            ...
            List<SessionDTO> sessionList = sessionBO.getAllSessions();
