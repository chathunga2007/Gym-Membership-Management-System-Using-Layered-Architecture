...
public class PrivateSessionController implements Initializable {
    ...
    private final PrivateSessionBO privateSessionBO = (PrivateSessionBO) BOFactory.getInstance().getBO(BOFactory.BOType.PRIVATE_SESSION);
    ...
    private void save() {
        ...
        if(!timeIn.matches(TIME_IN_REGEX)) {
            ...
            try {
                ...
                boolean result = privateSessionBO.saveWithPayment(privateSessionDTO, paymentDTO);
                if (result) {
    ...
    private void update() {
        ...
        if(!timeIn.matches(TIME_IN_REGEX)) {
            ...
            try {
                ...
                boolean result = privateSessionBO.updatePrivateSession(privateSessionDTO);
                if (result) {
                    Alert alert = new Alert(Alert.AlertType.INFORMATION);
    ...
    private void delete() {
        ...
        try {
            boolean result = privateSessionBO.deletePrivateSession(id);
            if (result) {
    ...
    private void handleSearchSession(KeyEvent event) throws SQLException {
        if (event.getCode() == KeyCode.ENTER) {
            try {
                ...
                PrivateSessionDTO privateSessionDTO = privateSessionBO.searchPrivateSession(id);
    ...
    private void loadPrivateSessionTable() {
        try {
            List<PrivateSessionDTO> privateSessionList = privateSessionBO.getAllPrivateSessions();
    ...
    private void loadMemberID(){
        try {
            ObservableList<MemberDTO> idList = privateSessionBO.loadMemberIDs();
