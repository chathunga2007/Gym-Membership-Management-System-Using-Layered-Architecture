...
public class PasswordResetDAOImpl implements PasswordResetDAO {
    ...
    public void saveOTP(String email, String otp) throws SQLException {
        CrudUtil.execute("DELETE FROM password_reset WHERE email=?", email);

        CrudUtil.execute("INSERT INTO password_reset (email, otp, created_at) VALUES (?, ?, NOW())", email, otp);
    }
    ...
    @Override
    public void clearOTP(String email) throws SQLException {
        CrudUtil.execute("DELETE FROM password_reset WHERE email=?", email);
    }
}
